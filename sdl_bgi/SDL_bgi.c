...
int kbhit (void)
{
  ...
  if (SDL_PollEvent (&event)) {
    ...
    //else if(SDL_MOUSEBUTTONDOWN == event.type || SDL_MOUSEBUTTONUP == event.type || SDL_MOUSEWHEEL)
...
int xkbhit (void)
{
  ...
  if (SDL_PollEvent(&event))
  {
      ...
      else
      {
          ...
          {
            //if (SDL_MOUSEBUTTONDOWN == event.type || SDL_MOUSEBUTTONUP == event.type || SDL_MOUSEWHEEL == event.type)
              //  SDL_PushEvent(&event); // don't disrupt the mouse
          }
      }
  }
